...
class SmartPopover {
  ...
  // Disabled for now - position-area support is inconsistent across browsers
  // Force JS fallback for reliable positioning
  static supportsAnchorPositioning = false;
   ...
   */
  open(anchorElement) {
    console.log('530 SmartPopover: open() called with anchor:', anchorElement);
    if (this.isOpen) {
      console.log('530 SmartPopover: Already open, returning');
      return;
    }

    this.anchor = anchorElement;
    console.log('530 SmartPopover: Creating popover...');
    this._createPopover();
    console.log('530 SmartPopover: Popover created:', this.popover);
  ...
  _createPopover() {
    console.log('530 SmartPopover: _createPopover() starting');
    ...
    if (typeof this.content === 'function') {
      ...
      console.log('530 SmartPopover: Content from function, length:', contentHtml?.length);
      this.popover.innerHTML = contentHtml;
    } else {
      console.log('530 SmartPopover: Content is string, length:', this.content?.length);
      this.popover.innerHTML = this.content;
    }

    // Force base styles inline to ensure visibility (light theme)
    ...
    console.log('530 SmartPopover: Popover appended to body');
    console.log('530 SmartPopover: Popover computed styles - opacity:', window.getComputedStyle(this.popover).opacity, 'zIndex:', window.getComputedStyle(this.popover).zIndex);
  }
  ...
  _applyManualPositioning() {
    ...
    console.log('530 SmartPopover: Manual positioning - anchor:', anchorRect, 'popover:', popoverRect, 'viewport:', viewport);
    ...
    console.log('530 SmartPopover: Final position - top:', top, 'left:', left, 'placement:', actualPlacement);
  }
