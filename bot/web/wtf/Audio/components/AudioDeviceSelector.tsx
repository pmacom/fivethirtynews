...
export const AudioDeviceSelector: React.FC = () => {
  const audioDevices = useStoreValue(AudioStore, 'audioDevices'); // Get available audio devices
  const isSetup = useStoreValue(AudioStore, 'isAudioStreamSetup');
  ...
  const setupAudio = () => AudioStore.set('setupAudio');
  const stopAudio = () => AudioStore.set('stopAudioStream');

  useEffect(() => {
    AudioStore.set('listAudioDevices'); // Fetch the available audio devices
  }, []);

  const handleDeviceChange = (deviceId: string) => {
    setSelectedDevice(deviceId); // Update the selected device in the component state
    AudioStore.set('mergeState', { deviceId }); // Also set the deviceId in the store
  ...
  const connect = (deviceId: string) => {
    AudioStore.set('mergeState', { deviceId })
    setupAudio();
  }
