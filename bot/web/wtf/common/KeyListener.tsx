...
export const KeyListener = ({ enabled = true, children, onKeyLeft, onKeyRight, onKeyDown, onKeyUp, onKeySpace }: KeyListenerProps) => {
  ...
  const handleKeyDown = useCallback(
    ...
    [hovered, onKeyLeft, onKeyRight, onKeySpace, onKeyDown, onKeyUp]
  );
  ...
  const onMouseEnter = useCallback(() => {
    KeyListenerStore.set('isHovered', true)
    setHovered(true)
  }, [setHovered])

  const onMouseLeave = useCallback(() => {
    KeyListenerStore.set('isHovered', false)
    setHovered(false)
  }, [setHovered])
...
export const KeyListenerStore = createStore<KeyListenerStoreState>({
  isHovered: false,
}, {
  name: 'keylistener-store'
}).extendActions(({})=>({
}))
